...
func (wc *WebClaude2) GetOptions() types.Options {
	return wc.Options
}
...
func (wc *WebClaude2) getOrganization() error {
	...
	if uid, _ := result[0]["uuid"]; uid != nil && uid != "" {
		wc.Options.OrganizationId = uid.(string)
		return nil
	}
...
func (wc *WebClaude2) createConversation() error {
	...
	response, err := wc.newRequest(30*time.Second, http.MethodPost, "organizations/"+wc.Options.OrganizationId+"/chat_conversations", headers, params)
	if err != nil {
		return err
	}
...
func (wc *WebClaude2) PostMessage(timeout time.Duration, prompt string, attr *types.Attachment) (*models.Response, error) {
	...
	params["conversation_uuid"] = wc.Options.ConversationId
	params["organization_uuid"] = wc.Options.OrganizationId
	params["text"] = prompt
